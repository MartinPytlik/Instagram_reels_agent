...
def run_command(cmd, description):
    ...
    try:
        ...
        print("Příkaz překročil timeout")
        return ""
...
def main():
    ...
    if "List of devices attached" in output:
        ...
        if not lines or all(not line.strip() for line in lines):
            print("ZADNA ZARIZENI NENALEZENA")
            print("\nZkus:")
            ...
            for line in lines:
                if line.strip() and '\t' in line:
                    ...
                    if status == 'device':
                        print(f"OK  {serial}: ONLINE")
                    elif status == 'unauthorized':
                        print(f"!!  {serial}: UNAUTHORIZED")
            ...
            if not devices_found:
                print("ADB vidí něco, ale není to rozpoznané zařízení")

    print("\n" + "=" * 60)
    print("  Pokud stále nefunguje:")
    print("=" * 60)
